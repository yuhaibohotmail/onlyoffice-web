...
export type OpenCommand = {
  ...
  readOnly?: boolean;
  /**
   * 【2026-09-17 新增】编辑器界面上给不给下载 / 另存。不给＝允许（与加这一格之前一样）。
   *
   * 设为 `false`：文件菜单里没有「下载为」和另存面板；
   * 预览、编辑、以及宿主发 `save` 存回去都照常。
   *
   * ⚠ **它只关界面入口，挡不住有心人**：接入页要把原文件整个取到浏览器里才能打开，
   * 开发者工具里照样看得到那次请求。真要防拿走文件，得在后端那一侧想办法。
   */
