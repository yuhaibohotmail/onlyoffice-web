...
// ⚠ **要的是「源」，不是这套东西的基地址。** 挂在前缀下时 `静态` 是
// `http://127.0.0.1:3042/oow`，而接入页比的是 `event.origin`（永远不带路径）。
// 把带路径的那个交进去，对照组会永远不成立，
// 于是①那条「被忽略了」变成恒真——**一道守卫看着还在，其实什么都没在守**。
const 乙 = await 试一次(new URL(静态).origin);
断言(
