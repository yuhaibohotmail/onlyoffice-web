...
改了什么、什么时候改的，见 [NOTICE.zh.md](NOTICE.zh.md)——那份文件是许可要求的一部分，不是可选文档。

本项目自足：自己有一套完整的前端、后端、插件与自动实测，不连任何外部服务。

本仓库的文档以英文为准。部分文档旁边另有一份中文副本（`*.zh.md`，比如这一份），副本是可选的，两边不一致时照英文那份。
...
npm run precompress  # 给 vendor/ 里每个大文件配一份 .gz（33 秒；走网线的字节少一半）

npm run server     # 3041：取件 / 存件 / 伺服静态资源、插件与许可原件
npm run dev        # 3040：页面
npm run e2e        # 自动实测（19 条，判据全部取东西本身）
```

`npm run assets` 顺带按编辑器各生成一份预热页（单独重生成用 `npm run preload-pages`）。
`npm run precompress` 由 `npm run release` 自动跑，只在直接对着 `vendor/` 干活时才需要手动跑一次。
这两样加起来不是小数：一份文档冷载从 **221.7 MB 降到 58.5 MB**（FINDINGS 第十五节）。

几条随手能跑的检查。**前五条零设施、亚秒级**，后面几条要两个服务起着 + 真浏览器：
...
npm run check:preload-pages  # 每个编辑器都有一份预热页吗，还与它的 app.js 对得上吗
npm run check:precompress    # vendor/ 里每个大文件都配着不旧的 .gz 吗
...
这一类问题，读数在 [embed-poc/README.zh.md](embed-poc/README.zh.md)。
...
装出来的包里是**整棵源码树 + `dist/` + `vendor/` + 一份部署说明 `DEPLOY.md`（英文）**，
...
| `OOW_NO_PRECOMPRESSED` | 设成 `1` 就不发 `.gz` 那一份。**只为量数用**：它是「压缩到底省了多少」那一问的对照组 | 配着 `.gz` 而且对方收得下压缩的，就发那一份 |
...
那台 web 服务器要是直接发静态树而不是转给后端，就把 `gzip_static on; gzip_vary on;` 打开。
`.gz` 那一份发布包里已经带着，所以这一条在请求时零开销，而它是「第一次打开走 58.5 MB 还是 131.1 MB」的差别。
⚠ `gzip_static` **不受 `gzip_types` 管**——正因为如此它才盖得住那些字体文件：它们根本没有扩展名，而且是最大的一族。
⚠ 不开也不会坏，唯一的症状是流量翻倍，而没有任何东西会说一句。
...
| `src/` | **组件本体（AGPL-3.0），这个项目唯一要发布的那一份。** 本项目改过的地方都带以「【本项目修改」或「【本项目新增」开头的注释标记。组件的文档在 `src/docs/` |
...
| `embed-poc/` | **把这个组件嵌进别人页面里的示例**：宿主页用 iframe 装下编辑器，两边只经 `postMessage` 说话。它自足——不依赖本仓库其余任何代码，后台服务自带。同时是一套实测（`npm run poc:e2e`，七趟，其中一趟**期望红**）。读数与容易出错的地方见 [embed-poc/README.zh.md](embed-poc/README.zh.md) |
| `NOTICE.md` | 修改说明（许可要求的） |
| `LICENSE` | AGPL-3.0 全文 + Ascensio 的五条附加条款 |
| `poc/backend-x2t/` | ⚠ **作废，暂时原地留着。** 试过「后端也跑一份 wasm 转换引擎」那条中间路，裁定不做（理由见 [BACKEND.zh.md](BACKEND.zh.md)）。⚠ 它几个脚本里读 `fixtures/lesson-plan-zh.docx` 的那几行**已经指不到东西了**——夹具 2026-08-30 搬进了 `demo/`，作废的东西没跟着改。那不是它坏了，是路径过期了 |
...
**自动实测 19 条全过**（`npm run e2e`）：取件那道门、从服务端取件画首屏、真键盘打字、
存回服务端且磁盘上的字节真的变了、没票不许写、插件面板 / 插公式 / 文档内按钮 / 配置下发四样、
关掉登记的对照组、官方插件登记、两种 PDF 各去各的应用、编辑器与查看器是两个不同的应用、
只预热这一种文档要用的那个编辑器、预压缩副本真的发出去了。

| 事 | 状态 |
|---|---|
| 静态资源从社区版镜像抽取 | ✅ 5 个编辑器应用全在（含 PDF 与 Visio），`sdkjs-plugins` 19 项全在 |
| 官方插件能用 | ✅ 镜像自带那 11 个已登记上，且每条登记都取得到（上游那份登记表是空壳，那正是「插件用不了」的第一个原因） |
| PDF 打得开 | ✅ **两种 PDF 各去各的地方**：普通 PDF 进 `pdfeditor`（看和批注），OnlyOffice 自己生成的**可编辑 PDF** 进 `documenteditor`（能填能改）。后者原本会被当成普通 PDF 打开、丢掉可编辑性且不报错，2026-08-30 修好，见 [FINDINGS.zh.md](FINDINGS.zh.md) 第十节 |
...
| 格式转换引擎 | ⚠ 用的是 CryptPad 发布的现成产物（校验和已核，比上游那份小 30%），**基于 core 9.3.0.140，与我们的 9.4.0.129 差一档**。⚠ **这一档差价是有代价的**：换上基于更新 core 的 wasm 一试，`epub` 与 `html` 就都好了。自己编要一台 Linux 加 docker，见 [build/x2t/README.zh.md](build/x2t/README.zh.md) |
...
| 一次打开到底下多少东西 | ✅ **冷载 58.5 MB / 387 请求**，从 221.7 MB / 437 降下来的（暖载两边都是 91 KB）。两步，各自量过：预热 iframe 改成只拉这次要用的那一个编辑器、不再四个一起（省 90.6 MB），每个大文件旁边配一份 `.gz` 由服务器发出去（省 72.6 MB）。⚠ **中文字体现在占剩下的 82%**（48.2 MB）——文档声明了哪个字族就整份下，而且只压得掉四成上下。`npm run measure:payload` 现量一遍，整笔账见 FINDINGS 第十五节 |
| `mobile` / `forms` 两个入口 | ⚠ 仍然没用上，也没有一条实测碰过。`variant` 那一格只开放了 `embed` 那一档 |
| 发布 | ⚠ **能装出一个跑得起来的整套了**（`npm run release`，实测把它当目标机跑通：文档打开、插件面板 2 个、导出 25518 字节、四条许可链接齐全）。代码已经发到公开仓库（<https://github.com/yuhaibohotmail/onlyoffice-web>）。**但还没有部署到任何机器上**，也没有 CI |
| 合规五条 | 见下 |
...
| 2 | 修改要声明（含日期、说明基于 ONLYOFFICE） | ✅ [NOTICE.zh.md](NOTICE.zh.md)，界面入口里也列着 |
