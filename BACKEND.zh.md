...
> 3. **DS 不是备选方案，是现成的东西**：凡是已经部署了 ONLYOFFICE Document Server（9.4.0.1）
>    并且可用的地方，它就是现成的文档引擎。
>    而且开销量过：**50 个并发编辑连接只让容器多吃 30 MiB**（每连接 0.29–0.32 MiB），
>    25 人持续打字占一台 2 核测试机 3.4% CPU——测量记录里的原话是
>    「先撑不住的是测试台，不是 DS」。
>
...
**它的路径名是照一个真实的文档管理后端的接口形状取的**（`server/index.mjs` 头注释明说），
也就是说：**A 那一块属于承载这个组件的那个应用自己的文档管理后端，不属于这个项目**——
那类后端的 auth / tenant / document / storage / version / editor / audit / webhook 那一整套，
...
不是抽象缺点，是已经写在 README.zh.md「已知的限制」里的三条，加上 PoC 新量出来的两条：
...
| 4 | 接字体，服务端出 PDF | 不做。等于在 Node 里重写 doctrenderer，去复刻一个 **Document Server 里现成就有的**二进制 |
| 5 | 预览图 / 缩略图 | 不做。同上；且单独的预览服务（例如 kkFileView）本来就做得了预览 |
...
产品那一侧的宿主集成是宿主应用自己的文档管理后端的活。

**三、许可这条与做不做后端无关，仍然成立。** `src/` 整棵树是 AGPL-3.0。
只要这个组件被拿去给别人用（第八节那几种情况全是），
README.zh.md 那两条不许越过的线就一直有效——尤其第一条「接受了 AGPL 就要真的做到」，
界面上那个法律声明入口是硬要求。
...
按轻重排的话，DS 实测根本不重（50 并发多吃 30 MiB），这条轴分不出东西来。
...
| 能容忍 | 不能容忍 |
|---|---|
| 用 **DS**。开销量过；在已经部署了 DS、宿主的文档后端也已经接上它的地方，一分新工作都没有 | 用**浏览器端那份**。代价是没有协作、四个格式有缺口、每人下 36 MB |

「不能容忍」具体是哪几种情况，值得写死，否则这个项目会因为没有边界而慢慢什么都想做：

1. **要把编辑器发给别人**——第三方或别的团队引用这个组件，不能要求对方先部署一个 1.3 GB 的容器。
   这是可复用前端组件包的形状，也是本项目 README.zh.md 第一句写的定位。
2. **纯静态托管**：产物拷到任意静态服务器就能跑，没有后端进程。
3. **离线或断网演示**。
4. 在宿主应用里嵌一个**只读预览**，不想为一次预览走「建会话 → 发令牌 → 挂回调」那一整套。

⚠ **第 4 条今天最需要重新掂量。** `NEXT-SESSION-PROMPT.md` 里排在第一件的「加查看器选项」
就是冲它去的，而它的前提是「宿主应用拿不到 DS」——**凡是已经有 DS 可用的地方，这个前提就不成立**。
还没部署 DS 的环境，补装一套是部署配置上的事，不是一项新工程。
